...
-- JOBS PROGRAMADOS PARA GDPR Y MANTENIMIENTO DE SEGURIDAD
-- OpenEnergies CRM - Migración de Seguridad
-- ============================================================================
-- Cumplimiento: GDPR Art. 5(1)(e)/17, ISO 27001 A.12.4.1, NIS2 Art. 21, SOC 2 CC6.1
...
-- DESCRIPCIÓN
-- ============================================================================
-- Este script configura jobs de pg_cron para:
-- 1. Procesar solicitudes de eliminación GDPR pendientes
-- 2. Limpiar logs de auditoría antiguos respetando retención
-- 3. Limpiar registros de rate limiting
...
DECLARE
    ...
    v_solicitud RECORD;
BEGIN
    -- Procesar solicitudes vencidas (> 30 días)
    FOR v_solicitud IN
...
-- ============================================================================
-- SECCIÓN 3: FUNCIÓN PARA LIMPIAR LOGS DE AUDITORÍA
-- ============================================================================
...
DECLARE
    ...
    v_deleted_storage BIGINT := 0;
    v_resultado JSONB;
BEGIN
    -- Limpiar logged_actions según configuración
    WITH deleted AS (
    ...
    -- Limpiar storage_access_log (retención: 1 año)
    WITH deleted AS (
        DELETE FROM audit.storage_access_log
    ...
    v_resultado := jsonb_build_object(
        ...
        'deleted_storage_logs', v_deleted_storage,
        'ejecutado_en', current_timestamp
    );
    
    -- Registrar si hubo eliminaciones
    IF v_deleted_actions > 0 OR v_deleted_security > 0 OR v_deleted_storage > 0 THEN
        INSERT INTO audit.security_events (
            event_type, event_timestamp, success, metadata
        ) VALUES (
...
DECLARE
    ...
    v_resultado JSONB;
BEGIN
    -- Eliminar registros de rate limiting mayores a 1 hora
    WITH deleted AS (
...
-- ============================================================================
-- SECCIÓN 5: FUNCIÓN PARA VERIFICAR INTEGRIDAD DIARIA
-- ============================================================================
...
BEGIN
    ...
    WHERE NOT EXISTS (
        SELECT 1 FROM public.documentos d WHERE d.storage_path = o.name
    )
    ...
    WHERE n.nspname = 'public'
      ...
      AND c.relname NOT IN ('rate_limit_log', 'storage_bucket_config');  -- Excepciones conocidas
    ...
    v_resultado := jsonb_build_object(
        ...
        'login_stats', (
            SELECT jsonb_build_object(
                'successful', COUNT(*) FILTER (WHERE event_type = 'LOGIN_SUCCESS'),
                'failed', COUNT(*) FILTER (WHERE event_type = 'LOGIN_FAILED'),
                'blocked', COUNT(*) FILTER (WHERE event_type = 'LOGIN_BLOCKED')
            )
            FROM audit.security_events
            WHERE event_timestamp BETWEEN v_period_start AND v_period_end
              AND event_type IN ('LOGIN_SUCCESS', 'LOGIN_FAILED', 'LOGIN_BLOCKED')
        ),
...
DECLARE
    ...
    v_resultado JSONB;
BEGIN
    -- Marcar usuarios con contraseñas muy antiguas (> 90 días)
    UPDATE public.usuarios_app
    SET 
    ...
    WHERE (password_changed_at IS NULL OR password_changed_at < (current_timestamp - INTERVAL '90 days'))
      ...
      AND rol = 'administrador'  -- Solo forzar en admins
...
-- ============================================================================
-- IMPORTANTE: Ejecutar estas sentencias como superusuario o desde el dashboard

-- Eliminar jobs existentes si hay que recrearlos
...
SELECT cron.schedule(
    ...
    '0 2 * * *',  -- Cron: minuto hora día-mes mes día-semana
...
COMMENT ON VIEW public.vista_cron_jobs IS 
'Vista para monitorear el estado de los jobs programados de OpenEnergies CRM.';
...
BEGIN
    ...
    CASE p_job_name
        ...
        ELSE
            RAISE EXCEPTION 'Job no reconocido: %', p_job_name;
    END CASE;
