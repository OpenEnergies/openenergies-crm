-- ============================================================================
-- 008_actualizar_funcion_trigger_v2.sql
-- Sistema de Auditoría y Actividad - Función Trigger con Columnas Denormalizadas
-- Proyecto: Open Energies CRM
-- Fecha: 2026-01-18
-- ============================================================================

-- Actualizar la función trigger para incluir punto_id y contrato_id
...
BEGIN
    ...
    -- Inicializar IDs denormalizados
    v_punto_id := NULL;
    v_contrato_id := NULL;
    
    -- Resolver cliente_id, punto_id y contrato_id según el método especificado
    CASE v_metodo_cliente
        WHEN 'direct' THEN
            ...
            ELSIF v_entidad_tipo = 'punto' THEN
                -- Es un punto de suministro
        ...
        WHEN 'via_punto' THEN
            ...
            -- También guardamos punto_id
            IF TG_OP = 'DELETE' THEN
                v_punto_id := OLD.punto_id;
                v_contrato_id := v_entidad_id; -- El contrato es la entidad
        ...
        WHEN 'via_contrato' THEN
            ...
            IF TG_OP = 'DELETE' THEN
                ...
                SELECT c.punto_id, ps.cliente_id INTO v_punto_id, v_cliente_id
                FROM contratos c
        ...
        WHEN 'documento_cascada' THEN
            -- Documentos: intentar resolver cliente_id, punto_id, contrato_id en cascada
            IF TG_OP = 'DELETE' THEN
    ...
    -- Insertar el registro de actividad con columnas denormalizadas
...
COMMENT ON FUNCTION fn_registrar_actividad() IS 'Función trigger genérica para registrar eventos de auditoría en actividad_log. Incluye punto_id y contrato_id denormalizados para filtrado jerárquico.';
