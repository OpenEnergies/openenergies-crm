// src/components/agrupaciones/AgrupacionFacturas.tsx
// Facturas table scoped to multiple puntos (agrupación)
...
// ============ FETCH FUNCTION ============
async function fetchFacturasByPuntos(puntoIds: string[]): Promise<FacturaCliente[]> {
...
export default function AgrupacionFacturas({ puntoIds }: { puntoIds: string[] }) {
  ...
  const puntoIdsKey = useMemo(() => [...puntoIds].sort().join(','), [puntoIds]);

  const { data: facturas = [], isLoading, isError } = useQuery({
    queryKey: ['agrupacion-facturas-table', puntoIdsKey],
    queryFn: () => fetchFacturasByPuntos(puntoIds),
    enabled: puntoIds.length > 0,
  });
  ...
  const getSignedUrl = useCallback(async (facturaId: string, comercializadoraNombre: string | null | undefined, numeroFactura: string): Promise<string | null> => {
    ...
    setSignedUrlCache(prev => ({
      ...prev,
      [facturaId]: { url: data.signedUrl, expires: Date.now() + (SIGNED_URL_EXPIRY - 30) * 1000 },
    }));
  ...
  const handlePreviewPdf = useCallback(async (factura: FacturaCliente) => {
    ...
    try {
      ...
      if (!url) { toast.error('PDF no encontrado en almacenamiento'); return; }
  ...
  const { sortedData, handleSort, renderSortIcon } = useSortableTable<FacturaCliente>(filteredFacturas, {
    ...
    sortValueAccessors: {
      ...
      cups: (item: FacturaCliente) => item.puntos_suministro?.cups || '',
    },
  });
  ...
  if (puntoIds.length === 0) {
    return (
      <EmptyState
        ...
        title="Sin puntos"
        description="No hay puntos en esta agrupación. Añade puntos para ver sus facturas."
      />
    );
  }
  ...
  if (isError) {
    return (
      <div className="glass-card p-6 text-red-400" role="alert">
        Error al cargar las facturas de la agrupación.
      </div>
    );
  }
    ...
    <div className="glass-card overflow-hidden">
      ...
      <div className="p-6 border-b border-bg-intermediate">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-fenix-600 dark:text-fenix-500">Facturas de la Agrupación</h2>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="flex items-center gap-2 flex-1 md:flex-initial">
              ...
              <input
                type="text"
                placeholder="CUPS, nº factura, comercializadora..."
                value={searchTerm}
      ...
      {displayedData.length === 0 ? (
        <EmptyState
          icon={<FileText className="text-fenix-500" size={48} />}
          title="Sin facturas"
          description={searchTerm ? 'No se encontraron facturas con ese criterio de búsqueda.' : 'Los puntos de esta agrupación no tienen facturas registradas.'}
        />
      ) : (
          ...
          <table className="w-full text-left">
            <thead>
              ...
              >
                ...
                <th className="p-4">
                  <button onClick={() => handleSort('numero_factura' as any)} className="flex items-center gap-1 hover:text-fenix-400 transition-colors cursor-pointer">
                ...
                <th className="p-4">
                  <div className="flex items-center gap-2">
                    <button onClick={() => handleSort('fecha_emision' as any)} className="flex items-center gap-1 hover:text-fenix-400 transition-colors cursor-pointer">
            ...
            <tbody className="divide-y divide-fenix-500/10">
              {displayedData.map(factura => (
                <tr key={factura.id} className="hover:bg-fenix-500/8 transition-colors">
                  <td className="p-4">
                    <code className="text-xs font-mono text-fenix-600 dark:text-fourth">{factura.puntos_suministro?.cups ?? '—'}</code>
                  </td>
                  <td className="p-4">
                    <button
                  ...
                  <td className="p-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      >
                        {loadingPdfId === factura.id ? <Loader2 size={16} className="animate-spin" /> : <Eye size={16} />}
                      </button>
                      <button
      ...
      {/* Pagination */}
      {displayedData.length > 0 && (
        <div className="p-4 flex flex-col sm:flex-row items-center justify-between gap-4" style={{ borderTop: `2px solid ${tableBorderColor}` }}>
          ...
          <div className="flex items-center gap-2">
            <button className="p-2 rounded-lg hover:bg-bg-intermediate text-gray-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors cursor-pointer" onClick={() => setCurrentPage(1)} disabled={currentPage === 1} title="Primera página"><ChevronsLeft size={18} /></button>
            <button className="p-2 rounded-lg hover:bg-bg-intermediate text-gray-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors cursor-pointer" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} title="Página anterior"><ChevronLeft size={18} /></button>
            <button className="p-2 rounded-lg hover:bg-bg-intermediate text-gray-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors cursor-pointer" onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} title="Página siguiente"><ChevronRight size={18} /></button>
            <button className="p-2 rounded-lg hover:bg-bg-intermediate text-gray-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors cursor-pointer" onClick={() => setCurrentPage(totalPages)} disabled={currentPage === totalPages} title="Última página"><ChevronsRight size={18} /></button>
          </div>
        </div>
      )}
