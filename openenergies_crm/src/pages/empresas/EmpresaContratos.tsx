  ...
  if (isLoading) {
    return (
      <div className="glass-card p-6 flex items-center justify-center gap-3">
        ...
        <span className="text-gray-400">Cargando contratos...</span>
      </div>
    );
  }
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-bg-intermediate">
              ...
              <th className="p-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">FV</th>
              <th className="p-4 text-center text-xs font-semibold text-gray-400 uppercase tracking-wider">Cobrado</th>
              <th className="p-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Activación</th>
              <th className="p-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Renovación</th>
            </tr>
          </thead>
          <tbody>
            {contratos.map((contrato) => (
              <tr key={contrato.id} className="border-b border-bg-intermediate hover:bg-bg-intermediate transition-colors cursor-pointer">
                <td className="p-4">
                  {contrato.cliente ? (
                    <Link
                      to="/app/clientes/$id"
                ...
                <td className="p-4 text-center">
                  {contrato.cobrado ? (
                    ...
                    <Circle size={18} className="text-gray-500 mx-auto" />
                  )}
                </td>
                <td className="p-4 text-gray-400">{fmtDate(contrato.fecha_activacion)}</td>
                <td className="p-4 text-gray-400">{fmtDate(contrato.fecha_renovacion)}</td>
              </tr>
            ))}
          </tbody>
        </table>
