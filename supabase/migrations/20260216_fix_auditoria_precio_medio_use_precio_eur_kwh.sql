-- Fix: precio_medio_kwh in Block A now uses weighted average of precio_eur_kwh
-- instead of SUM(total)/SUM(consumo_kwh) which included fixed costs and taxes.
-- Block D: precio_medio field now also uses precio_medio_energia instead of
-- precio_medio_efectivo for consistency.
...
BEGIN
    ...
    -- Precio medio = media ponderada de precio_eur_kwh por consumo
    -- (NO coste/consumo, que incluye costes fijos e impuestos)
    -- =====================================================
    ...
    resumen_mensual AS (
        SELECT 
            ...
            -- Precio medio ponderado por consumo usando precio_eur_kwh
            CASE 
                WHEN SUM(consumo_kwh) > 0 
                THEN ROUND((SUM(COALESCE(precio_eur_kwh, 0) * consumo_kwh) / SUM(consumo_kwh))::NUMERIC, 4)
                ELSE 0 
    ...
    agrupado_por_tarifa AS (
        SELECT 
            ...
            -- Precio medio global por tarifa: media ponderada por consumo
            CASE
                WHEN SUM(consumo_total) > 0
                THEN ROUND((SUM(precio_medio_kwh * consumo_total) / SUM(consumo_total))::NUMERIC, 4)
                ELSE 0
            END AS precio_medio_tarifa
    ...
    -- =====================================================
    -- BLOQUE D: Facturación Agregada por Punto
    -- Precio medio efectivo y precio medio energía ponderado
    -- =====================================================
    ...
    SELECT COALESCE(
        jsonb_agg(
            jsonb_build_object(
                ...
                'consumo_total', consumo_total,
                'coste_total', coste_total,
                'precio_medio', precio_medio_energia,
                'precio_medio_energia', precio_medio_energia
            ) ORDER BY cups
        ),
        '[]'::jsonb
